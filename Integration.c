/*
 * main_emw3080.c
 * Register-level STM32U585 driver for EMW3080 over SPI2
 *
 * Board:  B-U585I-IOT02A
 * Chip:   STM32U585
 * Module: MXCHIP EMW3080B
 *
 * Pin mapping (verified against board schematic):
 *   PF15  OUTPUT          Chip_En  (WRLS.WKUP_W)
 *   PD3   AF5  SPI2_MISO  (WRLS.SPI2_MISO)
 *   PD4   AF5  SPI2_MOSI  (WRLS.SPI2_MOSI)
 *   PD1   AF5  SPI2_SCK   (WRLS.SPI2_SCK)
 *   PB12  OUTPUT          CS       (WRLS.SPI2_NSS)
 *   PG15  INPUT           FLOW     (WRLS.FLOW)   HIGH = module ready
 *   PD14  INPUT           NOTIFY   (WRLS.NOTIFY) HIGH = module has data
 *   PG6   OUTPUT          WKUP_B   (WRLS.WKUP_B)
 *   PA9   AF7  USART1_TX
 *   PA10  AF7  USART1_RX
 *
 * EMW3080 SPI frame protocol (from official mx_wifi_spi.c):
 *   Header (8 bytes, #pragma pack(1)):
 *     [0]    type   : 0x0A = master write, 0x0B = master read
 *     [1..2] len    : uint16_t little-endian, payload length
 *     [3..4] lenx   : ~len  (integrity check: len ^ lenx == 0xFFFF)
 *     [5..7] dummy  : ignored
 *   Master always sends type=0x0A.
 *   Slave always responds with type=0x0B in its header.
 *   Integrity check: (slave_len ^ slave_lenx) must equal 0xFFFF.
 *   FLOW pin handshake:
 *     1. CS LOW
 *     2. Wait FLOW HIGH  -> exchange 8-byte header
 *     3. Wait FLOW HIGH  -> exchange max(master_len, slave_len) data bytes
 *     4. CS HIGH
 *
 * STM32U5 SPI differences from older STM32 SPIs (RM0456):
 *   - CR2.TSIZE must be set to the exact byte count before CSTART.
 *     If TSIZE=0 the peripheral runs in indefinite mode; EOT never fires
 *     and the state machine behaves unpredictably in master mode.
 *   - CSTART fires one burst of exactly TSIZE bytes then stops.
 *   - For back-to-back header + data transfers you must:
 *       a) wait for EOT (SR.EOT) after the first burst,
 *       b) clear EOT via IFCR.EOTC,
 *       c) set new TSIZE and CSTART for the second burst.
 *   - SSI must be HIGH (CR1.SSI=1) before setting MASTER in CFG2
 *     to prevent a spurious MODF fault.
 *   - SPE must be set AFTER CR1/CFG1/CFG2 are configured.
 *   - CSTART must be set AFTER SPE.
 */
